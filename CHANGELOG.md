...
- **A portable ZIP is built beside the Windows installer.** The extracted copy carries its own
  marker and keeps settings, caches and logs in a `data` folder beside the executable, without
  reading or migrating the installed profile.

- **Every release can be checked against VirusTotal by its own hash.** The README, the FAQ and the
  troubleshooting guide now show how to print an installer's SHA-256 and open the matching report,
  so a download can be verified without taking the project's word for it.

- **The project website is easier to discover and follow.** Search engines receive a sitemap,
  robots policy and structured application data; readers get a styled changelog and an RSS release
  feed generated from this file.
...
- **Games added through LumaPlay come back.** Every one of them vanished from the library: the reader
  ...
  PC, and the third-party server that used to be asked first - and sent a copy of every schema
  generated here - is gone.

- **Xbox achievements carry the date they were unlocked, and their progress.** Xbox answers with a
  date written out in full, which was being read as a number: every Xbox unlock was stored with no
  date at all. The counters of a "collect 100 of these" achievement were fetched and cached, then
  dropped before the game screen could paint their bar.

- **A save folder created after AW Next started is watched straight away.** A game creates its
  folder the first time it runs, and only folders that already existed were being watched, so the
  very first achievement of a newly installed game went unnoticed until the next restart.

- **"Skip this version" is no longer undone by the next change made in Settings.** The window keeps
  its own copy of the settings for as long as it stays open, and saving anything wrote that copy back
  over what the update prompt had recorded.

- **A scan with no connection no longer takes minutes.** Several sources kept asking a host that had
  just proved unreachable, once per game, each waiting out its own timeout: the Epic, GOG, Ubisoft,
  Exophase and Steam lookups, the emulator tool downloads and the automatic fix list now stop asking
  after the first failures and answer from what is already on disk.

- **A game emulated by a GBE Fork build that ships its configuration templates unrenamed is read
  correctly.** It was taken for a classic Goldberg setup, so its saves were looked for in the wrong
  folder and a working game reported having none.

- **A Ubisoft game the store itself installed is left alone by the automatic repair.** The check that
  refuses to touch a registered Ubisoft Connect folder was applied when repairing by hand but not
  when repairing on its own.

- **The library no longer re-downloads what it already knows is missing.** The memo that remembers an
  AppID Steam has nothing for depended on a Steam endpoint that has since been retired, so it was
  never written: every scan paid the full lookup for those games again.

- **A game with several sources keeps the progress of its counters.** An achievement whose counter
  sat at zero was left with no value at all rather than zero, so its progress bar disappeared from
  the in-game overlay.

- **One unreadable folder no longer ends the whole scan.** A folder that could not be read stopped
  every folder after it, across every emulator, with one line in the log.

- **Playtime cannot be corrupted by the clock.** A session was measured against the wall clock, so an
  automatic time correction while playing could add a negative duration to the total and turn it into
  a nonsense figure.
...
- **Archives are checked before they are opened.** Every emulator package, fix archive and tool
  download is now read through the same check, which refuses an entry that would write outside the
  folder it is being extracted into. Two of the five did no checking at all.

- **A corrupted game index repairs itself.** The file that remembers what each game is was kept as-is
  when it could not be read, and playtime and Game Health then stayed blind to every game for good.
  It is now set aside and rebuilt by the next scan.
...
- **The window is on screen more than twice as fast.** The startup path had never been measured past
  the point where the window already existed, and four things were hiding in front of it: the
  updater was read in full before anything opened, so was the whole achievement parser, the sweep
  for leftover monitors blocked on a `netstat` call that can take over a second, and the window then
  asked for its own settings over that blocked channel. Each of them now happens when it is needed,
  or beside the window instead of in front of it. Measured on a cold profile, 1.5 s to 0.6 s, and
  the work the main process does before it can open a window went from 3 s to under 0.1 s.
- **The parser loads 104 files where it loaded 260.** The hashing and XML libraries, like the search
  and download engines before them, are read when something actually parses a game. The zip readers
  behind theme and preset import went the same way.
