...
- **Import or replace DLLs** / **Restore integrated DLLs** - use your own loader build, or go back to
  the one AW Next ships.

Every changed DLL, schema and INI is snapshotted under `<game>\.aw-backups\<timestamp>\` and restored
automatically if anything fails validation afterward. Repeating an identical repair is a no-op.
