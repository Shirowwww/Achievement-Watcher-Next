...
| **Automatic achievement data updates** | Switch, on by default. Turning it off stops that automatic recheck and the blank-description repair, for anyone who maintains `steam_cache` by hand. **Recheck achievement lists** still works as a one-off check while it is off. |
...
Right-click a game whose folder name matches several Steam releases - so the automatic name match
picked the wrong one - and choose **Set AppID manually...**. The choice is remembered per install
folder and written to `steam_appid.txt`; **Clear the manual AppID override** undoes it. Game
Health's **Correct the game ID file** action now asks for the AppID too, starting from AW Next's own
suggestion instead of only confirming it.
...
**Remove AW Next's emulator configuration**, in the same right-click menu, undoes only the DLC
section, switches and identity keys AW Next itself wrote to a game's `configs.user.ini` - never a
